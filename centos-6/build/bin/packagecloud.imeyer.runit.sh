...
yum_repo_path=/etc/yum.repos.d/imeyer_runit.repo
yum_repo_config_url="https://packagecloud.io/install/repositories/imeyer/runit/config_file.repo?os=${os}&dist=${major_version}&name=${host}&source=script"
...
yum install -y pygpgme --disablerepo='imeyer_runit'
...
if [ "$?" != "0" ]; then
  ...
  sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/imeyer_runit.repo
fi

echo "Installing yum-utils..."
yum install -y yum-utils --disablerepo='imeyer_runit'
...
echo "Generating yum cache for imeyer_runit..."
yum -q makecache -y --disablerepo='*' --enablerepo='imeyer_runit'
